...
class ResourceLoader {
				...
				const result = await Promise.race([
					...
					new Promise<never>((_, reject) =>
						setTimeout(
							() =>
								reject(
									new Error(`Timeout ao carregar recurso: ${url}`),
								),
							timeoutMs,
						),
					),
	...
	private static _loadFont(url: string): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			if (!fontName) {
				reject(
					new Error('Nome da fonte não pôde ser extraído do URL'),
				);
				return;
			}
	...
	private static _loadGeneric(url: string): Promise<string> {
		return fetch(url).then((res) => {
			if (!res.ok)
				throw new Error(
					`Erro ao carregar recurso: ${res.statusText}`,
				);
			return res.text();
		});
	}
